...
PowerShell 包装脚本也支持调用，推荐使用 `-Arg key=value`（可重复）来规避 JSON 引号转义问题：

```powershell
./scripts/call_mcp_tool.ps1 -Tool what_to_eat -Arg people_count=3
./scripts/call_mcp_tool.ps1 -Tool recommend_meals -Arg people_count=3 -Arg allergies='["虾"]'
```

兼容旧参数 `-ArgsJson`，但在 PowerShell 下容易受到引号转义影响：

```powershell
./scripts/call_mcp_tool.ps1 -Tool what_to_eat -ArgsJson '{"people_count":3}'
```
